...
export function CompanyTableToolbar({ filters, options, onResetPage }: Props) {
  ...
  // Add safe defaults with null checks
  const safeFilters = {
  ...
  const handleFilterIndustry = useCallback(
    (event: SelectChangeEvent<string[]>) => {
      const newValue =
        typeof event.target.value === 'string' ? event.target.value.split(',') : event.target.value;

      onResetPage();
      updateFilters({ industry: newValue });
    },
    [onResetPage, updateFilters]
  );
    ...
    <>
      ...
      >
        <FormControl sx={{ flexShrink: 0, width: { xs: 1, md: 200 } }}>
          <InputLabel htmlFor="filter-industry-select">Industry</InputLabel>
          <Select
            multiple
            value={safeFilters.industry}
            onChange={handleFilterIndustry}
            input={<OutlinedInput label="Industry" />}
            renderValue={(selected) => selected.map((value) => value).join(', ')}
            inputProps={{ id: 'filter-industry-select' }}
            MenuProps={{ PaperProps: { sx: { maxHeight: 240 } } }}
          >
            {safeOptions.industries.map((option) => (
              <MenuItem key={option} value={option}>
                <Checkbox
                  disableRipple
                  size="small"
                  checked={safeFilters.industry.includes(option)}
                />
                {option}
              </MenuItem>
            ))}
